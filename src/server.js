...
import {
  ...
  cloudState,
  Gun
} from './vendors/commChannels';
import Renderer from 'hypernova-client';
import devModePlugin from './vendors/devModePlugin';
...
if (HYPER_URL) {
  ...
  app.get('/s/aphrodite', (req, res) => {
    const jobs = {
      aphrodite: { name: req.query.menu || HYPER_URL }
    };
    renderer.render(jobs).then(html => res.send(html))
  });
  app.get('/s/simple', (req, res) => {
    const jobs = {
      SimpleComponent: { name: req.query.name || HYPER_URL }
    };
    renderer.render(jobs).then(html => res.send(html));
  });
}
